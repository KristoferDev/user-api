...
app.put('/', (req, res) => {
   connection.query('UPDATE users SET last_name = ?, first_name = ? WHERE user_id = ?', ['Karlsson', 'Kalle', 3], function (error, results, fields) {
      if (error) throw error;
...
app.delete('/', (req, res) => {
   connection.query('DELETE FROM users WHERE user_id = 3', function (error, results, fields) {
      if (error) throw error;
...
app.post('/', (req, res) => {
   var post  = {user_id: 3, last_name: 'Svensson', first_name: 'Sven', mobile: 70478372, email: 'sven@gmail.com', age: '30', gender: 'm'};
